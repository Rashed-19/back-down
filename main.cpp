...
int main(){
	...
	system("title Bar Management System");
	...
	head = NULL;
	insertfirst(5,"Vodka Shots",23,120.23);
	insertend(6,"tequilla",13,100.67);
	insertend(1,"Virgin Mojito",8,720.83);
	insertend(2,"Cocktails",46,70.23);
	insertend(3,"Kiwi Cocktails",46,70.23);
		...
		if(main_menu_choice == 1){
			...
			if(admin_panel_choice==1234567){
				...
				if(adminchoise==1){
					...
					br(3);pre(4);      printf(" Enter bar item Name :  ");
					...
					br(2);pre(4);  printf(" Enter Item Serial :  ");
                      ...
                      if(exist->data==fdata){
                       cls(); br(5);pre(3);  printf(" Bar Item Serial Already Exist, Please Re-Enter  "); Sleep(2000);
                       goto foodserial;
                      }
					...
					cls();
					middle1();pre(2);
					printf("Enter Serial No of the Bar Item To Delete : ");
					fdelete:
					int fdelete;
	...
	if(get_food_choice == temp->data){
		...
		if(confirm == 1 ){
			...
			if(payment==1){
				...
				br(2);pre(4);  printf("1. Wanna Take Another Shot ? ");
...
void main_menu(){
	...
	cls();
	br(5); pre(3); echo(">> 1. Bar Item List"); Sleep(400);
